...
/* Объект используется для хранения в виде полей cookies пользователей, 
которые прошли авторизовались и имеют доступ к панели администратора */
let authCookies = {};

/*Данный обработчик будет запускаться всякий раз, когда пользователь будет делать запрос начинающийся с /admin. 
Обработчик проверяет, если пользователь запрашивает панель администратора, то он его пропускает, 
иначе проверяет доступ, и если его нет перенаправляет пользователя на путь /admin. */
route.use((req, res, next)=>{//Проверяем запрашивает ли пользователь панель администратора '/admin'
	if(req.originalUrl === '/admin'){
		next(); //Передаём управление следующим обработчикам
	} else {/*Пользователь не запрашивает панель администратора, поэтому проверяем есть ли cookies разрешающая доступ к этой части сайта*/
...
Внутри осуществляется проверка логина и пароля. 
Если проверка пройдена, в ответном запросе устанавливаем специальную cookies и перенаправляем на страницу /admin. 
Если проверка не пройдена возвращаем страницу аутентификации с сообщением об ошибке */
route.post('/', (req, res, next)=>{
	...
	/*Запрашиваем в хранилище пользователей конкретного пользователя по присланному логину, 
	в функцию обратного вызова вернёться ошибка или объект описывающий пользователя (переменная user) */
...
/*Роут организующий прием запрос о выходе текущего пользователя из системы. Полный путь: /admin/out. 
Внутри осуществляется удаление ранее установленной cookie и удаление пометки, разрешающей пользователям с этой cookie иметь доступ к этой части сайта*/
